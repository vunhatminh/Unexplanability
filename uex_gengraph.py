...
def gen_propagate_mechanics_1(input_dim = 10, nb_shapes=80, width_basis=300, m=5, perturb_prob = 0.01):
    """ Synthetic Graph #7:

    Start with Barabasi-Albert graph and attach cycle-of-6-nodes subgraphs.

    Args:
        nb_shapes         :  The number of shapes (here 'cycle') that should be added to the base graph.
        width_basis       :  The width of the basis graph (here a random 'ba').
        feature_generator :  A `FeatureGenerator` for node features.
    ...
    """
    basis_type = "ba"
    list_shapes = [["cycle6"]] * nb_shapes


    G, role_id, plugins = uex_synthetic_structsim.build_graph(
        width_basis, basis_type, list_shapes, start=0
    )
    G = perturb([G], perturb_prob)[0]
    
    feat_vals = []

    for i in range(width_basis):
        feat_vals.append(role_id[i])
        
    node_pos = 0
    for i in range(width_basis, nb_shapes* 6 + width_basis):
        if node_pos > 5:
            node_pos = node_pos - 6
            
        if node_pos + 3 > 5:
            feat_vals.append(role_id[i + 3 - 6])
        else:
            feat_vals.append(role_id[i + 3])
            
        node_pos = node_pos + 1 
    
    feat_gen = uex_featgen.SynP1RoleGen(np.asarray(feat_vals), input_dim = input_dim)
    feat_gen.gen_node_features(G)

    name = basis_type + "_" + str(width_basis) + "_" + str(nb_shapes)

    return G, role_id, name

def gen_propagate_mechanics_2(input_dim = 10, nb_shapes=80, width_basis=300, m=5, perturb_prob = 0.01):
    """ Synthetic Graph #7:

    Start with Barabasi-Albert graph and attach tree-of-6-nodes subgraphs.
...
def gen_aggregate_mechanics_1(input_dim = 10, nb_shapes=80, width_basis=300, m=5, perturb_prob = 0.01):
    """ Synthetic Graph #7:
    ...
    feat_gen = uex_featgen.SynA1RoleGen(np.asarray(role_id), input_dim = input_dim)
    feat_gen.gen_node_features(G)
    
    labels = []
    for i in range(width_basis):
        labels.append(role_id[i])
        
    for c in range(nb_shapes):
        cluster = range(width_basis + c*6, width_basis + (c+1)*6)
        vals = []
        for node in cluster:
            vals.append(np.mean(G.nodes[node]['x'][7:input_dim]))
        max_item = max(vals)
        max_idx = vals.index(max_item)
        label = np.where(G.nodes[cluster[0] + max_idx]['x'][:7] == 1)[0][0]
...
def gen_aggregate_mechanics_2(input_dim = 10, nb_shapes=80, width_basis=300, m=5, perturb_prob = 0.01):
    """ Synthetic Graph #7:
...
def gen_syn1(nb_shapes=80, width_basis=300, feature_generator=None, m=5):
	...
	G, role_id, _ = uex_synthetic_structsim.build_graph(
		width_basis, basis_type, list_shapes, start=0, m=5
	)
	G = perturb([G], 0.01)[0]

	if feature_generator is None:
		feature_generator = uex_featgen.ConstFeatureGen(1)
...
#             [1.0] * 2 + random_mu), np.array([0.5] * 2 + random_sigma)
# 	feat_gen_G1 = uex_featgen.GaussianFeatureGen(mu=mu_1, sigma=sigma_1)
# 	feat_gen_G2 = uex_featgen.GaussianFeatureGen(
